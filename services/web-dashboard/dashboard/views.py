...
def _resolve_discord_id(token: str, payload: dict) -> str:
    ...
    for identity in supabase_user.get("identities") or []:
        ...
        discord_id = _extract_discord_id_from_identity_data(identity.get("identity_data") or {})
        if discord_id:
            return discord_id

    return _extract_discord_id_from_identity_data(supabase_user.get("user_metadata") or {})
...
def players(request):
    ...
    data = PlayerRankingSerializer(qs, many=True).data
    ...
    for i, p in enumerate(sorted_data):
        p['rank'] = i + 1
        p['pointsDiff'] = sorted_data[0]['totalPoints'] - p['totalPoints'] if i > 0 else 0
        ...
        running_acts = [
            a for a in user_obj.activity_set.all()
            if a.activity_type in ('bieganie_teren', 'bieganie_bieznia')
        ]
        paces = [pv for pv in (_pace_to_float(a.pace) for a in running_acts) if pv is not None]
...
def stats_summary(request):
    ...
    running_acts = [a for a in acts if a.activity_type in ('bieganie_teren', 'bieganie_bieznia')]
    running_paces = [pv for pv in (_pace_to_float(a.pace) for a in running_acts) if pv is not None]
    avg_running_pace = round(sum(running_paces) / len(running_paces), 2) if running_paces else None

    payload = {
        "avg5Points": int(avg5_points),
        "bestActivityPoints": int(best_activity.total_points) if best_activity else 0,
        "bestActivityDate": best_activity.created_at.strftime('%Y-%m-%d') if best_activity else None,
...
def stats_distribution(request):
    ...
    return JsonResponse(ActivityDistributionSerializer(payload, many=True).data, safe=False)
...
def admin_events(request):
    ...
    if request.method == 'GET':
        events = AirsoftEvent.objects.prefetch_related('registrations__user').all().order_by('start_date', 'id')
        data = AsgEventSerializer(events, many=True).data
        return JsonResponse(list(data), safe=False)

    body = json.loads(request.body or '{}')
    date_raw = body.get('date')
    start_date = timezone.make_aware(datetime.strptime(date_raw, '%Y-%m-%d')) if date_raw else timezone.now()
...
def admin_event_detail(request, event_id: int):
    ...
    if 'participants' in body:
        ...
        users = DiscordUser.objects.filter(discord_id__in=(body.get('participants') or []))
        for u in users:
...
def admin_challenge_detail(request, challenge_id: int):
    ...
    if 'startDate' in body:
        challenge.start_date = timezone.make_aware(datetime.strptime(body['startDate'], '%Y-%m-%d'))
    if 'endDate' in body:
        challenge.end_date = timezone.make_aware(datetime.strptime(body['endDate'], '%Y-%m-%d'))
    if 'isActive' in body:
...
def admin_activities(request):
    ...
    if search:
        qs = qs.filter(Q(user__display_name__icontains=search) | Q(user__discord_id__icontains=search))
