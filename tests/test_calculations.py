...
def test_bieganie_teren_basic():
    ...
    points, error = orchestrator.calculate_points("bieganie_teren", 10)
...
def test_bieganie_bieznia_basic():
    ...
    points, error = orchestrator.calculate_points("bieganie_bieznia", 10)
    assert error == "", f"Nie powinno być błędu: {error}"
...
def test_plywanie_basic():
    """Test dla pływania."""
    points, error = orchestrator.calculate_points("plywanie", 2)
    assert error == "", f"Nie powinno być błędu: {error}"
...
def test_rower_min_distance():
    ...
    points, error = orchestrator.calculate_points("rower", 5)
    assert (
...
def test_rower_valid_distance():
    ...
    points, error = orchestrator.calculate_points("rower", 20)
    assert error == "", f"Nie powinno być błędu: {error}"
...
def test_spacer_valid_distance():
    ...
    points, error = orchestrator.calculate_points("spacer", 5)
    assert error == "", f"Nie powinno być błędu: {error}"
...
def test_plywanie_no_bonuses():
    """Test - pływanie nie powinno mieć bonusów (bonusy są ignorowane)."""
    points, error = orchestrator.calculate_points("plywanie", 2, weight=5, elevation=100)
    # Bonusy są ignorowane, więc nie ma błędu, ale punkty są tylko za dystans
    assert error == "", f"Nie powinno być błędu: {error}"
    # 2km * 4000 = 8000 (bonusy są ignorowane)
    assert points == 8000, f"Oczekiwano 8000 (bonusy ignorowane), otrzymano {points}"
...
def test_cardio_basic():
    """Test dla cardio."""
    points, error = orchestrator.calculate_points("cardio", 5)
    assert error == "", f"Nie powinno być błędu: {error}"
